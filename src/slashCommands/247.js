export default {
  ...
  execute: async function({ guild, client, respond, send }) {
    ...
    if (!await client.db.get247(guild.id)) {
      client.db.set247(true, guild.id).then(() => respond({ content: '<:yes2:835498559805063169> Режим 24/7 включён.', ephemeral: true }))
      if (client.timers.has(guild.id))
        clearTimeout(client.timers.get(guild.id))
    } else {
      client.db.set247(false, guild.id).then(() => respond({ content: '<:no2:835498572916195368> Режим 24/7 выключён.', ephemeral: true }))
      if (player)
